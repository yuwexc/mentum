...
export const Banner = ({ className }: { className?: ReactNode }) => {

    const { profile } = usePage<PageProps>().props;

    if (profile && profile.user.banner) {
        return <div className={`w-full rounded-md ` + className}
            style={{
                backgroundImage: `url(${profile.user.banner})`,
    ...
    const onSubmit: SubmitHandler<z.infer<typeof formSchema>> = (data) => {
        form.clearErrors();
        router.post(route('user.banner'), data, {
            onError: (errors) => {
