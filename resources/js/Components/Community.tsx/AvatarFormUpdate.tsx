...
export const AvatarFormUpdate = () => {
    ...
    const [previewUrl, setPreviewUrl] = useState<string | null>(community!.community.avatar);
    ...
    const onSubmit: SubmitHandler<z.infer<typeof formSchema>> = (data) => {
        form.clearErrors();
        router.post(route('community.update.avatar', {
    ...
    const handleDeleteAvatar = () => {
        form.clearErrors();
        setPreviewUrl(null);
        router.delete(route('community.avatar.delete', {
            community: community?.community.slug || community?.community.id
        }), {
            onError: (errors) => {
                Object.entries(errors).forEach(([field, message]) => {
        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className={`w-full flex flex-col gap-4 items-start justify-center`}>
                ...
                <div className="flex flex-wrap gap-4">
                    ...
                    {
                        community && community.community.avatar &&
                        <AlertDialog>
